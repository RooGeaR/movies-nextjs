...
const Modal = ({ open, onClose, children }: ModalType) => {
  return (
<div id="default-modal" tabIndex={-1} aria-hidden="true" className={`overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 flex justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] transition-colors max-h-ful ${open ? "visible bg-black/20" : "invisible"}`} onClick={onClose}>
    <div className="relative p-2 md:p-4 w-full max-w-5xl max-h-full">
